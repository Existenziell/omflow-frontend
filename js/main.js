...
om = {
    ...
    initMap: function() {
        // Create the map
        mapboxgl.accessToken = om.accessToken;
        om.map = new mapboxgl.Map({
            container: 'map',
    ...
    createMarkers: function() {
        om.places.features.forEach(function(marker) {
    ...
    forwardGeocoder: function(query) {
        ...
        for (var i = 0; i < om.places.features.length; i++) {
            var feature = om.places.features[i];
            if (
                feature.id
                .toLowerCase()
